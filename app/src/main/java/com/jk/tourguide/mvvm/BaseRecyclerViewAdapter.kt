...
abstract class BaseRVAdapter<T>(
    ...
    private var resource: Resource<List<T>>
    ...
    open fun createLoadingViewHolder(parent: ViewGroup): RecyclerView.ViewHolder {
        val view =
            ItemLoadingBinding.inflate(LayoutInflater.from(parent.context), parent, false).root
        return LoadingItem(view)
    }

    open fun createErrorViewHolder(parent: ViewGroup): RecyclerView.ViewHolder {
        val view = ItemErrorBinding.inflate(LayoutInflater.from(parent.context), parent, false).root
        return ErrorItem(view)
    }

    open fun createEmptyViewHolder(parent: ViewGroup): RecyclerView.ViewHolder {
        val view = ItemEmptyBinding.inflate(LayoutInflater.from(parent.context), parent, false).root
        return EmptyItem(view)
    }
